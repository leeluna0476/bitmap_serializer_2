...
struct	bmp_info_header_t
{
	...
	uint16_t	colour_plane;			// the number of colour planes (must be 1)
	uint16_t	bits_per_pixel;			// the number of bits per pixel, which is the colour depth of the image.
	...
	uint32_t	colour_number;			// the number of colours in the colour palette, or 0 to default to 2n
	uint32_t	colour_number_important;	// the number of important colours used, or 0 when every colour is important; generally ignored
};
#pragma pack(pop)
